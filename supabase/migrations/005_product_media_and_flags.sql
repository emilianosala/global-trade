-- Adds product image + curation flags, and upgrades get_products() to:
--   * expose the new columns,
--   * filter a category by its whole subtree (parent + descendants),
--   * optionally filter by the curation flags (for home sections).
-- Re-runnable: columns use IF NOT EXISTS and the function is dropped/recreated.
...
-- Signature changes (new params), so drop the old overload before recreating.
DROP FUNCTION IF EXISTS public.get_products(uuid, uuid);
...
BEGIN
  ...
  WHERE (p_category_id IS NULL OR pr.category_id IN (SELECT id FROM cat_tree))
